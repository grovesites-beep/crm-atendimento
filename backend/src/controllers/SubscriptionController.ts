...
export const createSubscription = async (
  ...
  // Busca configuração global (empresa admin ou .env)
    ...
    const response = await axios.post(
      ...
      data,
      {
        headers: {
          ...
          Authorization: `Bearer ${mpAccessToken}` // Usando token vindo do GetGlobalConfig
        }
      }
    );
  ...
  if (data && data.id) {
    try {
      ...
      if (paymentDetails.status === "approved") {
        const invoiceID = paymentDetails.additional_info.items[0].title.replace(
          "#Fatura:",
          ""
        );
